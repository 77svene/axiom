...
from axiom.core.custom_types import TextHandler
from axiom.engines.toolbelt.custom import Response
from axiom.core.utils._shell import _ParseHeaders, _CookieParser
...
class NoExitArgumentParser(ArgumentParser):  # pragma: no cover
    ...
    def exit(self, status=0, message=None):
        if message:
            log.error(f"axiom shell exited with status {status}: {message}")
            self._print_message(message, stderr)
        raise ValueError(f"axiom shell exited with status {status}: {message or 'Unknown reason'}")
...
class CurlParser:
    ...
    def __init__(self) -> None:
        from axiom.fetchers import Fetcher as __Fetcher
    ...
    def parse(self, curl_command: str) -> Optional[Request]:
        ...
        return Request(
            ...
            cookies=cookies,
            proxy=proxies,
            follow_redirects=True,  # axiom default is True
        )
    ...
    def convert2fetcher(self, curl_command: Request | str) -> Optional[Response]:
        if isinstance(curl_command, (Request, str)):
            ...
            if method in self._supported_methods:
                ...
                log.error(f'Request method "{method}" isn\'t supported by axiom yet')
                return None
...
def show_page_in_browser(page: Selector):  # pragma: no cover
    ...
    try:
        fd, fname = make_temp_file(prefix="axiom_view_", suffix=".html")
