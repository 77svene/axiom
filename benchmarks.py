...
from axiom import Selector as axiomSelector
...
def test_lxml():
    return [
        ...
        for e in etree.fromstring(
            large_html,
            # axiom and Parsel use the same parser inside, so this is just to make it fair
...
def test_axiom():
    ...
    return axiomSelector(large_html, adaptive=False).css(".item::text").getall()
...
def display(results):
    ...
    axiom_time = results["axiom"]
    print("\nRanked Results (fastest to slowest):")
    print(f" i. {'Library tested':<18} | {'avg. time (ms)':<15} | vs axiom")
    print("-" * 50)
    for i, (test_name, test_time) in enumerate(sorted_results, 1):
        compare = round(test_time / axiom_time, 3)
        print(f" {i}. {test_name:<18} | {str(test_time):<15} | {compare}")


@benchmark
def test_axiom_text(request_html):
    return axiomSelector(request_html, adaptive=False).find_by_text("Tipping the Velvet", first_match=True, clean_match=False).find_similar(ignore_attributes=["title"])
...
if __name__ == "__main__":
    ...
    results2 = {
        "axiom": test_axiom_text(req.text),
